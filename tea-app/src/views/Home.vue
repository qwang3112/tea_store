      ...
      <van-col span="24">
        <van-tabs @click="onClick" sticky title-active-color="#E32DAB" color="#E32DAB" :line-width="100" :line-height="2">
          ...
          <van-tab v-for="index in categories.length" :title="categories[index-1].name" class="tab">
            ...
            <van-card v-for="(item,index) in teas"
                      :price="item.price"
                      :desc="item.desc"
                      :title="item.title"
                      :thumb="item.thumb"
...
<script>
  ...
  export default {
    ...
    created(){
      const _this = this
      axios.get('http://localhost:8181/tea/index').then(function (resp) {
        _this.teas = resp.data.data.teas
    ...
    methods: {
      onClick(index) {
        const _this = this
        axios.get('http://localhost:8181/tea/findByCategoryType/'+this.categories[index].type).then(function (resp) {
          _this.teas = resp.data.data
        })
      },
      buy(index){
        this.show = true
        const _this = this
        axios.get('http://localhost:8181/tea/findSpecsByTeaId/'+this.teas[index].id).then(function (resp) {
